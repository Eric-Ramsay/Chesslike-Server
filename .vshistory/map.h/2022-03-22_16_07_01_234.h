...
void rainSpread(std::vector< std::vector<Tile >>& map) {
	for (int m = 4; m > 0; m--) {
		for (int h = 1; h >= 0; h--) {
			int i = h * (MAPSIZE - 1);
			while (i < MAPSIZE && i >= 0) {
...
void currents(std::vector<std::vector<MapTile>>& map, bool fir, int id) {
	...
	auto makeCurrent = [&](int yC, int xC) {
		...
		while (cS < limit && water > 0) {
			...
			bool stayCourse = false; // (rand() % limit < cS);
...
std::vector<std::vector<Tile>> createMap(int seed, int num_tries = 0) {
	...
	for (int i = 0; i < MAPSIZE / 5; i++) {
		map[(MAPSIZE * .3) + rand() % MAPSIZE * .4][(MAPSIZE * .3) + rand() % MAPSIZE * .4].elevation = 80 + MAPSIZE / 2.5;
	}
	elevSpread(map);
	elevSpread(map);
	elevSpread(map);
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (map[i][j].elevation < 0) {
				if (rand() % 14 == 0) {
					if (map[i][j].type == 'a' || map[i][j].type == 'r' || rand() % 2 == 0) {
						bool adj_fish = false;
						for (int a = -4; a < 5; a++) {
							for (int b = -4; b < 5; b++) {
