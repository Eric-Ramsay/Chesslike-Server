...
void currents(std::vector<std::vector<MapTile>>& map, bool fir) {
	float ticks = 0;
...
std::vector<std::vector<Tile>> createMap(int seed) {
	...
	for (int i = 0; i < MAPSIZE/5; i++) {
		map[(MAPSIZE * .3) + rand() % MAPSIZE*.4][(MAPSIZE * .3) + rand() % MAPSIZE*.4].elevation = 125;
	}
	elevSpread(map);
	elevSpread(map);
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (rand() % MAPSIZE == 0) {
				map[i][j].forestChance = 300;
			}
		}
	}

	//genOceans(MAPSIZE * MAPSIZE, 1, map);
	slap(map, 0, 50, 'g', 'w', -100, 0);
	//slap(map, 0, 25, 'g', 'w', -100, 1);
	//slap(map, 500, 9000, 'g', 'w', -100, 1);
	std::cout << "Oceans Generated" << std::endl;
	mountain(map, 10*MODI, (int)round(80.0 * MODI));
	mountain(map, (int)(25 * MODI), (int)round(20.0 * MODI));
	//std::cout << "Mtns Generated" << std::endl
	hill(map, 25*MODI, (int)round(20.0 * MODI));
	hill(map, 10*MODI, (int)round(25.0 * MODI));
	hill(map, 5*MODI, (int)round(25.0 * MODI));
	//std::cout << "Hills Generated" << std::endl;
	elevSpread(map);
	elevSpread(map);
	forestSpread(map);
	//std::cout << "Elevation Generated" << std::endl;
	int dry_air = 2;// rand() % 4;
	//dry_air = (2 * (rand() % 2)) + 1;
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			else if (map[i][j].type == 'a' || map[i][j].type == 'r') {
				if (rand() % 4 == 0) {
					map[i][j].forest = 's';
				}
			}
