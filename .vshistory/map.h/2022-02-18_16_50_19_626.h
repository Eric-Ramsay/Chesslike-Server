...
std::vector<std::vector<Tile>> createMap(int seed) {
	...
	for (int i = 0; i < 8; i++) {
		C c;
		c.x = (MAPSIZE * .25) + rand() % MAPSIZE / 2;
		c.y = (MAPSIZE * .25) + rand() % MAPSIZE / 2;
		spots.push_back(c);
	}
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			//map[i][j].elevation += rand() % 5 - ((MAPSIZE/15)*fDist(j, i, MAPSIZE / 2, MAPSIZE / 2));
			if (map[i][j].elevation < 0) {
				map[i][j].type = 'w';
			}
			map[i][j].heat = latTemp(i);
			if (map[i][j].type == 'g') {
				map[i][j].elevation += 4 - (rand() % 10);
			}
