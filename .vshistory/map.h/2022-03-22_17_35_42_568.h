			...
			while (i < MAPSIZE && i >= 0) {
				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						if (map[i][j].elevation >= 70) {
							map[i][j].elevation = (rand() % 3 + ((2 * Max + avg) / 3 * (.91))) * (100 + pow(MODI, 2)) / 100.0;
						}
						if (map[i][j].elevation >= 50) {
							map[i][j].elevation = (rand() % 3 + ((Max + 2 * avg) / 3 * (.91))) * (100 + pow(MODI, 2)) / 100.0;
						}
						else if (Min < 20) {
							map[i][j].elevation = rand() % 5 + (Max + map[i][j].elevation + Min) / 3;
						}
						else {
							map[i][j].elevation = rand() % 3 + (.93 * (Max + avg) / 2);
						}
					}
					else {
						avg += 2*min(Max, 0);
