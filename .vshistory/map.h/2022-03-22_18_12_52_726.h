				...
				while (j < MAPSIZE && j >= 0) {
					...
					if (map[i][j].elevation > 0) {
						...
						if (Min < 20) { //Coastal
							...
							if (map[i][j].elevation < Max) { //highest elev around
								if (map[i][j].elevation >= 60) { //prob hill
									map[i][j].elevation = .90 * (Max*2 + avg) / 3;
								}
								if (map[i][j].elevation >= 50) { //prob hill
									map[i][j].elevation = .90 * (Max + avg) / 2;
								}
								else if (map[i][j].elevation >= 30) {
									map[i][j].elevation = .91 * (Max + 3*avg) / 4;
								}
								else {
									map[i][j].elevation = .91 * avg;
								}
							}
							else {
								map[i][j].elevation = rand() % 3 + .99 * (Max*9 + avg)/10;
							}
...
void currents(std::vector<std::vector<MapTile>>& map, bool fir, int id) {
	...
	auto makeCurrent = [&](int yC, int xC) {
		...
		while (cS < limit && water > 0) {
			...
			int maxX = 0;
			//stay course
			if (map[y][x].elevation <= 0 && rand() % limit < cS) {
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (a * b == 0 && a != b) {
