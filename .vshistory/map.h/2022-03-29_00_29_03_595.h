...
int NUM_THREADS = 4;
unsigned long long int ocean_water = 0;
unsigned long long int river_water = 0;
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (t.elevation > 0) {
				...
				else if (map[i][j].rain < 300) {
					map[i][j].type = 'p';
				}
				else if (map[i][j].rain < 400) {
					map[i][j].type = 'v';
				}
...
std::vector<std::vector<Tile>> createMap(int seed, int num_tries = 0) {
	...
	std::cout << "Ocean: " << ocean_water << std::end;
	lakes(map);
	rivers(map);
	for (int i = 0; i < 4; i++) {
		currents(map, true, i);
	}
	std::cout << "Rivers: " << ocean_water << std::end;
	//rainSpread(map);
	finalize(map);
