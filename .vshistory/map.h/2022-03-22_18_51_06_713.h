					...
					if (map[i][j].elevation > 0) { //land tiles
						...
						if (Min < 20) { //tile near coastal area
							...
							if (map[i][j].elevation >= 60) {
								map[i][j].elevation = (Max / 2 + avg / 2) * .93;
							}
							else if (map[i][j].elevation >= 50) {
								map[i][j].elevation = (((Max / 2 + avg / 2) * (.89)));
							}
							else if (map[i][j].elevation >= 30) {
								map[i][j].elevation = Max * (.75 + (rand() % 200) / 1000);
							}
							else {
								map[i][j].elevation = Max * (.75 + (rand() % 21) / 100.0);
								map[i][j].elevation = (Max * (.75 + ((((double)rand() / RAND_MAX)) * .2)));
							}
						}
