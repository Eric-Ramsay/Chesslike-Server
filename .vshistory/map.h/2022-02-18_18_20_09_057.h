...
void currents(std::vector<std::vector<MapTile>>& map, bool fir, int num) {
	float ticks = 0;
	...
	float avgLen = 0;
	int start = num % 4;
	for (int i = start; i < MAPSIZE; i += 4) {
		for (int j = start; j < MAPSIZE; j += 4) {
			switch (map[i][j].type) {
			case 'w': if (fir) {
				makeCurrent(i, j);
			} break;
