...
//For some reason this fixes usage of max and min in map.h
using namespace std;
#include <vector>
#include "classes.h"
#include "functions.h"
...
void serverInit() {
	...
	std::cout << "Server started at " << std::endl << "IP: " << IP << "Port: " << Port << std::endl;
	server_join();
				...
				for (Player p : players) { //Check which player sent the msg
					if (p.socket == current) {
						...
						if (bytesIn <= 0) { //The player DC'd
							...
							else if (c == 'p') { //player is sending their list of policies
								for (int i = 0; i < msg.length(); i++) {
									players[p.turn].policies[i] = (msg[i] - '0');
								}
								sendData("p1" + sender + msg);
							}
							else if (c == 'e' || c == 'E') {
