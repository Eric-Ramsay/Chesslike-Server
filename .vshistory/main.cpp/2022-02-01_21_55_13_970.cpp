					...
					if (p.socket == current) {
						...
						if (bytesIn <= 0) { //The player DC'd
							...
							if (c == 'u') { //Unit Change
								...
								if ((sender[0] - '0') == turn || c == 'E') { //End the current players turn. If, 'E', admin is overriding
...
void server_send() {
	...
	auto last_time = time;
	std::chrono::duration<float> aTime = (m_tp2 - m_tp1).count();
	...
	while (connected) {
		...
		aTime += chrono::duration_cast<chrono::milliseconds>(time - last_time).count();
		last_time = time;
