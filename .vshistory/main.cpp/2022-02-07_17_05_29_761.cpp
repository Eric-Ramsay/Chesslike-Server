...
void buildingChange(std::string s) {
	...
	std::cout << "Setting tile " + to_str(x) + ", " + to_str(y) + " to ";
	switch (map[y][x].building) {
	case 1: std::cout << "Stage 1 Farm" << std::endl; break;
	case 2: std::cout << "Stage 2 Farm" << std::endl; break;
	case 3: std::cout << "Stage 3 Farm" << std::endl; break;
	case 4: std::cout << "Wheat" << std::endl; break;
...
void serverInit() {
	...
	//noticable seeds 
	//	-> mega flat, size 44, seed 947
	// //size 50
	//  -> forest flat, size 50, seed=4016
	// -> seed = 28127
	// -> seed = 14845
	// -> seed = 2298 -> river maze
	// -> seed = 14422
	// -> seed = 13063 -> islands
	// -> seed = 4957 -> forest seclusion
			...
			if (current == listening) { //A new Player is connecting -----------
				...
				for (Player p : players) { //Check which player sent the msg
					if (p.socket == current) {
						...
						int bytesIn = recv(current, buf, 600, 0);

						if (bytesIn <= 0) { //The player DC'd
							std::cout << "Player Disconnected" << std::endl;
							players[p.turn-1].active = false;
							std::cout << players[p.turn - 1].active << std::endl;
							closesocket(current);
							...
							if (c == 'u') { //Unit Change
								...
								buildingChange(msg);
								sendData("b1" + sender + msg);
							}
							else if (c == 'G') { //User is saying how much gold they have
								players[p.turn - 1].gold = std::stoi(msg);
								std::cout << "Got Gold: " << p.gold << std::endl;
							}
							else if (c == 'g') { //User is asking how much gold they have
								sendData("G0" + sender + to_str(p.gold));
								std::cout << "Sending Gold: " << p.gold << std::endl;
							}
							else if (c == 'e' || c == 'E') {
								if (p.turn == (turn+1) || c == 'E') { //End the current players turn. If, 'E', admin is overriding
									do {
										turn = (1 + turn) % players.size();
									} while (!players[turn].active);
									sendData("e00" + to_str(turn+1));
								}
								else if (c != 'E') { //Make sure player knows whose turn it is
									sendData("e0" + sender + to_str(turn+1));
								}
							}
							...
							else if (c == 'm') { //User is requesting mapsize and turn
								sendData(sendMAPSIZE(MAPSIZE, "0" + to_str(turn + 1)));
							}
							else if (c == 'n') { //Reset map
								seed = rand() % 200000;
								std::cout << "Seed - " << seed << std::endl;
								map = createMap(seed);
								for (int i = 0; i < map.size(); i++) {
									sendData(sendRow(i, "0" + to_str(turn + 1)));
								}
								sendData(sendMAPSIZE(MAPSIZE, "0" + to_str(turn + 1)));
							}
						}
	...
	while (connected) {
		...
		if (buffer.size() > 0) {
			if (timer == target) {
				...
				for (int i = 0; i < master.fd_count; i++) {
					...
					if (outSock != listening) {
						for (int j = 0; j < players.size(); j++) {
							if (players[j].active && players[j].socket == outSock && players[j].last <= messages) {
				...
				clear = true;
				for (int j = 0; j < players.size(); j++) {
					if (players[j].active && players[j].last <= messages) {
						clear = false;
					}
				}
				if (clear) {
