	...
	while (connected) {
		...
		for (int i = 0; i < socketCount; i++) {
			...
			if (current == listening) {
				...
				std::string turn = "t0" + to_str((++NUM_PLAYERS) % (MAX_PLAYERS + 1));
				send(new_client, turn.c_str(), turn.size() + 1, 0);

				//Send Map
				for (int i = 0; i < map.size(); i++) {
					//sendRow(i, "0" + to_str((NUM_PLAYERS) % (MAX_PLAYERS + 1)));
				}
				...
				// Receive message
				int bytesIn = recv(current, buf, 4096, 0);
