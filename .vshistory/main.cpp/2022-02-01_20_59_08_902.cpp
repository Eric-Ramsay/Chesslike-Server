...
void server_send() {
	...
	auto aTime = std::chrono::high_resolution_clock::now() - std::chrono::high_resolution_clock::now();
	float tTime = 1.0f / 30.0; //Increment timer 30 times a second

	while (connected) {
		aTime += (std::chrono::high_resolution_clock::now() - time);
