...
float fTargetFrameTime = 1.0f / 30.0;
int timer = 0;
float last_time = 0.0;
	...
	while (connected) {
		...
		if (buffer.size() > 0) {
			...
			for (int i = 0; i < master.fd_count; i++) {
				SOCKET outSock = master.fd_array[i]; //Loop through each socket which is ready to recv
				if (outSock != listening) {
					...
					if (!received) { //Send the msg to the client
						...
						if (ape != s.size() + 1) { //This literally has never happened ever
							last[index]--;
